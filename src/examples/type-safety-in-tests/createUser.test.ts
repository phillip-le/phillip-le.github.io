...
describe('createUser', () => {
  ...
  it('should send tracking event', async () => {
    ...
    expect(trackUserCreatedMultipleArgs).toHaveBeenCalledWith<[string, string]>('user-id', 'James');
  });

  it('should send tracking event with generated type', async () => {
    await createUser({ userId: 'user-id', name: 'James' });

    expect(trackUserCreated).toHaveBeenCalledWith<Parameters<typeof trackUserCreated>>({
      userId: 'user-id',
      name: 'James',
    });
